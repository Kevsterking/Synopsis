function Synopsis() {
  return <div id="diagram-root" style={{position: "relative", width: "1500px", height: "900px", backgroundColor: "rgb(51, 51, 51)", overflow: "hidden"}}>
    ...
    <div id="diagram-elements" style={{position: "absolute", border: "1px solid white", width: "300px", height: "300px", left: "50%", top: "50%"}}>
    </div>
  </div> 
}

function Diagram(root_element, canvas_element, elements_element, left_corner_coordinate) {
  ...
  */
  this.updateGrid = () => {
    ...
    for (let x = (this.translation.x - this.canvas_element.offsetWidth * 0.5) % 100 - 50; x < this.canvas_element.offsetWidth; x += 100) {
      this.canvasContext.moveTo(x, 0);
      this.canvasContext.lineTo(x, this.canvas_element.offsetHeight);
    }
    for (let y = (this.translation.y - this.canvas_element.offsetHeight * 0.5) % 100 - 50; y < this.canvas_element.offsetWidth; y += 100) {
    ...
    for (let x = (this.translation.x - this.canvas_element.offsetWidth * 0.5) % 100; x < this.canvas_element.offsetWidth; x += 100) {
      this.canvasContext.moveTo(x, 0);
      this.canvasContext.lineTo(x, this.canvas_element.offsetHeight);
    }
    for (let y = (this.translation.y - this.canvas_element.offsetHeight * 0.5) % 100; y < this.canvas_element.offsetWidth; y += 100) {
      this.canvasContext.moveTo(0, y);
      this.canvasContext.lineTo(this.canvas_element.offsetWidth, y);
    }
    
    //this.canvasContext.moveTo((this.translation.x - this.canvas_element.offsetWidth * 0.5), 0);
    //this.canvasContext.lineTo(x, this.canvas_element.offsetHeight);
  ...
  */
  this.setTranslation = (x, y) => {
    ...
    this.translation.x = Math.max(x, - this.offset.x - this.root_element.clientWidth * 0.5 - this.elements_element.offsetWidth + 100);
    this.translation.y = Math.max(y, - this.offset.y - this.root_element.clientHeight * 0.5 - this.elements_element.offsetHeight + 100);
    this.translation.x = Math.min(this.translation.x,  this.root_element.clientWidth * 0.5 - 100 - this.offset.x);
    this.translation.y = Math.min(this.translation.y,  this.root_element.clientHeight * 0.5 - 100 - this.offset.y);
    
    this.updateGrid();
    this.elements_element.style.transform = "translate("+this.offset.x+"px, "+this.offset.y+"px) translate("+this.translation.x+"px, "+this.translation.y+"px)";
...
window.onload = (event) => {
  ...
  let dia = new Diagram(diagram_root, diagram_canvas, diagram_elements, {x: 0, y: 0});
